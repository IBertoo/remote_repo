...
if (!is_dir($uploadDir) && !mkdir($uploadDir, 0777, true)) {
    die("❌ No se pudo crear la carpeta de subida en $uploadDir");
}
...
if (isset($_POST['delete_image_id'])) {
    ...
    try {
        ...
        if ($image) {
            ...
            $basename = basename($image['nombre_img']);
            ...
            $filePathMedium = $uploadDir . 'medium_' . $basename;
            if (is_file($filePathMedium)) {
                unlink($filePathMedium); // eliminar archivo físico
            }

            $filePathSmall  = $uploadDir . 'small_'  . $basename;
            if (is_file($filePathSmall)) unlink($filePathSmall);
            ...
            foreach ($uploadedFiles['tmp_name'] as $i => $tmpName) {
                ...
                if (move_uploaded_file($tmpName, $destination)) {
                    ...
                    $stmt = $pdo->prepare("
                        ...
                        VALUES (:pid, :fn)
                    ");
                    $stmt->execute([':pid' => (int)$id, ':fn' => 'uploads/' . $filename]);
